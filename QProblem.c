...
returnValue QProblem_determineHessianType( QProblem* _THIS )
{
	...
	for ( i=0; i<nV; ++i )
	{
		...
		if ( qpOASES_getAbs( curDiag - 1.0 ) > QPOASES_EPS2 )
			isIdentity = BT_FALSE;

		if ( qpOASES_getAbs( curDiag ) > QPOASES_EPS2 )
			isZero = BT_FALSE;
	}
												...
												)
{
	...
	*Delta_bB_isZero = BT_TRUE;
	...
	for ( i=0; i<nFX; ++i )
	{
		...
		if ( ( qpOASES_getAbs( delta_lb[ii] ) > QPOASES_EPS2 ) || ( qpOASES_getAbs( delta_ub[ii] ) > QPOASES_EPS2 ) )
		{
...
*/
	...
	for( iter=nWSRperformed; iter<*nWSR; ++iter )
	{
		...
		{
			if ( ( _THIS->tau <= QPOASES_EPS2 ) && ( _THIS->options.enableRamping == BT_TRUE ) )
				QProblem_performRamping( _THIS );
			else
...
returnValue QProblem_backsolveT( QProblem* _THIS, const real_t* const b, BooleanType transposed, real_t* const a )
{
	...
	if ( transposed == BT_FALSE )
	{
		...
		for( i=0; i<nT; ++i )
		{
			...
			if ( qpOASES_getAbs( TT(i,_THIS->sizeT-1-i) ) > QPOASES_EPS2 )
	...
	else
	{
		...
		for( i=0; i<nT; ++i )
		{
			...
			if ( qpOASES_getAbs( TT(nT-1-i,tcol+i) ) > QPOASES_EPS2 )
											...
											)
{
	...
	*Delta_bC_isZero = BT_TRUE;
	...
	for ( i=0; i<nAC; ++i )
	{
		...
		if ( ( qpOASES_getAbs( delta_lbA[ii] ) > QPOASES_EPS2 ) || ( qpOASES_getAbs( delta_ubA[ii] ) > QPOASES_EPS2 ) )
		{
		  								...
		  								)
{
	...
	switch ( _THIS->options.printLevel )
	{
		case PL_DEBUG_ITER:
			...
			/* complementarity */
			for (i = 0; i < nV; i++) if (_THIS->y[i] > +QPOASES_EPS2 && qpOASES_getAbs((_THIS->lb[i] - _THIS->x[i])*_THIS->y[i]) > bcmpl) bcmpl = qpOASES_getAbs((_THIS->lb[i] - _THIS->x[i])*_THIS->y[i]);
			for (i = 0; i < nV; i++) if (_THIS->y[i] < -QPOASES_EPS2 && qpOASES_getAbs((_THIS->ub[i] - _THIS->x[i])*_THIS->y[i]) > bcmpl) bcmpl = qpOASES_getAbs((_THIS->ub[i] - _THIS->x[i])*_THIS->y[i]);
			for (i = 0; i < nC; i++) if (_THIS->y[nV+i] > +QPOASES_EPS2 && qpOASES_getAbs((_THIS->lbA[i]-AX[i])*_THIS->y[nV+i]) > ccmpl) ccmpl = qpOASES_getAbs((_THIS->lbA[i]-AX[i])*_THIS->y[nV+i]);
			for (i = 0; i < nC; i++) if (_THIS->y[nV+i] < -QPOASES_EPS2 && qpOASES_getAbs((_THIS->ubA[i]-AX[i])*_THIS->y[nV+i]) > ccmpl) ccmpl = qpOASES_getAbs((_THIS->ubA[i]-AX[i])*_THIS->y[nV+i]);
											...
											)
{
	...
	if (lb_new && ub_new) {
		for (i = 0; i < QProblem_getNV(_THIS); ++i) {
			if (lb_new[i] > ub_new[i]+QPOASES_EPS2) {
	...
	if (lbA_new && ubA_new) {
		for (i = 0; i < QProblem_getNC(_THIS); ++i) {
			if (lbA_new[i] > ubA_new[i]+QPOASES_EPS2) {
				return RET_QP_INFEASIBLE;
			}
		}
	}
