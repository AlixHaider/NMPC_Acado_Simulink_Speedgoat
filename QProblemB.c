...
returnValue QProblemB_determineHessianType( QProblemB* _THIS )
{
	...
	for ( i=0; i<nV; ++i )
	{
		...
		if ( qpOASES_getAbs( curDiag - 1.0 ) > QPOASES_EPS3 )
			isIdentity = BT_FALSE;

		if ( qpOASES_getAbs( curDiag ) > QPOASES_EPS3 )
			isZero = BT_FALSE;
	}
											...
											)
{
	...
	*Delta_bB_isZero = BT_TRUE;
	...
	for ( i=0; i<nFX; ++i )
	{
		...
		if ( ( qpOASES_getAbs( delta_lb[ii] ) > QPOASES_EPS3 ) || ( qpOASES_getAbs( delta_ub[ii] ) > QPOASES_EPS3 ) )
		{
											...
											const real_t* const lb_new, const real_t* const ub_new )
{
	...
	if (lb_new && ub_new) {
		for (i = 0; i < QProblemB_getNV(_THIS); ++i) {
			if (lb_new[i] > ub_new[i]+QPOASES_EPS3) {
								...
								)
{
	...
	for( iter=nWSRperformed; iter<*nWSR; ++iter )
	{
		...
		/* 7) Perform Ramping Strategy on zero homotopy step or drift correction (if desired). */
		 if ( ( _THIS->tau <= QPOASES_EPS3 ) && ( _THIS->options.enableRamping == BT_TRUE ) )
